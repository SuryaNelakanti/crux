...
export async function upsertProblemMembers(
    rows: ProblemMember[]
): Promise<void> {
    if (rows.length === 0) return;
    const client = getUntypedClient();
...
export async function upsertUserProblemLogs(
    rows: UserProblemLog[]
): Promise<void> {
    if (rows.length === 0) return;
    const client = getUntypedClient();
...
export async function insertEvents(events: AppEvent[]): Promise<void> {
    ...
    const payload = events.map((event) => ({
        ...
        server_ts: toIso(event.serverTs),
    }));
...
export async function resolveProblemShareToken(
    token: string
): Promise<string | null> {
    const client = getUntypedClient();
...
export async function joinProblemWithToken(
    token: string
): Promise<string | null> {
    const client = getUntypedClient();
