...
async function uploadPendingMedia(): Promise<void> {
  ...
  for (const item of items) {
    ...
    try {
      ...
      await markOutboxMediaFailed(item.id, item.retryCount + 1, message);
    }
  }
}
...
async function pullRemoteUpdates(currentUserId: string): Promise<void> {
  ...
  const { data: problemsRowsData } = await client.from('problems').select('*').in('id', problemIds);
  ...
  const logsQuery = client.from('user_problem_logs').select('*').in('problem_id', problemIds);
