...
export interface OutboxEventDeps {
  getPendingOutboxEvents: () => Promise<{ id: string; eventId: string; retryCount: number }[]>;
  ...
  markOutboxEventFailed: (outboxId: string, retryCount: number, error: string) => Promise<void>;
}

export async function flushOutboxEvents(deps: OutboxEventDeps): Promise<void> {
  ...
  for (const item of items) {
    try {
      ...
      const message = error instanceof Error ? error.message : 'event flush failed';
      await deps.markOutboxEventFailed(item.id, item.retryCount + 1, message);
    }
  }
}
