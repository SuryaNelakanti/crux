...
describe('flushOutboxEvents', () => {
    it('inserts events and removes outbox items', async () => {
        const deps = {
            getPendingOutboxEvents: jest.fn().mockResolvedValue([
                { id: 'outbox-1', eventId: 'event-1', retryCount: 0 },
            ]),
    ...
    it('marks failures with incremented retry count', async () => {
        const deps = {
            getPendingOutboxEvents: jest.fn().mockResolvedValue([
                { id: 'outbox-2', eventId: 'event-1', retryCount: 2 },
            ]),
            getEventById: jest.fn().mockResolvedValue(baseEvent),
        ...
        expect(deps.markOutboxEventFailed).toHaveBeenCalledWith(
            'outbox-2',
            3,
            'boom'
        );
    ...
    it('drops items with missing events', async () => {
        const deps = {
            getPendingOutboxEvents: jest.fn().mockResolvedValue([
                { id: 'outbox-3', eventId: 'missing', retryCount: 0 },
            ]),
