...
export async function createProblemFromPhoto(params: {
  ...
  const metadata = params.metadataJson ? JSON.stringify(params.metadataJson) : null;
  ...
  await db.runAsync(
    `
        ...
        `,
    [problemId, localUserId, now.toISOString(), params.sessionId, mediaId]
  );
...
export async function getProblemById(problemId: string): Promise<{
  ...
  const media = row.media_id
    ? {
        ...
        width: row.width ?? 0,
        height: row.height ?? 0,
        createdAt: row.media_created_at ? new Date(row.media_created_at) : new Date(row.created_at),
        sha256: row.sha256,
        bytes: row.bytes,
