import { generateId, OUTBOX_MAX_RETRIES, type AppEvent } from '@crux/shared';
...
export async function enqueueOutboxMedia(
    mediaId: string,
    localPath: string
): Promise<void> {
    const db = await getDb();
...
export async function markOutboxMediaUploading(outboxId: string): Promise<void> {
    const db = await getDb();
    await db.runAsync(
        "update outbox_media set status = 'uploading' where id = ?",
        [outboxId]
    );
}
