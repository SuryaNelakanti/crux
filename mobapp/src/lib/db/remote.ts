import type { Media, Problem, ProblemMember, RouteMask, UserProblemLog } from '@crux/shared';
import { getDb } from './schema';

export async function upsertRemoteProblemMembers(members: ProblemMember[]): Promise<void> {
  ...
  for (const member of members) {
    await db.runAsync(
      `
            ...
            `,
      [member.problemId, member.userId, member.role, member.joinedAt.toISOString()]
    );
  }
}
...
export async function upsertRemoteUserProblemLogs(rows: UserProblemLog[]): Promise<void> {
  const db = await getDb();
