...
export async function getProblemCardsForSession(
    ...
    const problems = webStore.problems.filter(
        (problem) => problem.createdInSessionId === sessionId
    );

    return problems.map((problem) => {
        const media = webStore.media.find(
            (entry) => entry.id === problem.primaryMediaId
        );
...
export async function upsertUserProblemLog(params: {
    ...
    const existing = webStore.logs.find(
        (entry) =>
            entry.problemId === params.problemId &&
